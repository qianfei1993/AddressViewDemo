...
- (void)configSubLayersAndSubviews
{
    ...
    for (int i = 0; i < self.subTextLayers.count; i++) {
        ...
        CGFloat rightMargin;
        if (i < 2) {
            itemWidth = 2 * self.configuration.indexItemHeight;
            rightMargin = self.configuration.indexItemRightMargin;
        }else{
            itemWidth = self.configuration.indexItemHeight;
            rightMargin = self.configuration.indexItemRightMargin /2;
        }
        textLayer.frame = CGRectMake(self.bounds.size.width - rightMargin - self.configuration.indexItemHeight, SCGetTextLayerCenterY(section, margin, space) - self.configuration.indexItemHeight / 2, itemWidth, self.configuration.indexItemHeight);
...
- (void)onActionWithDidSelect
{
    ...
    if (self.isTouchingIndexView && self.configuration.isGenerator) {
